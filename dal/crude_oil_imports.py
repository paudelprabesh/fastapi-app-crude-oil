...
async def update_crude_oil_imports(db, update_data_model, filters) -> Optional[dict]:
    try:
        ...
        (
            update_data["destination_name"],
            update_data["destination_type_name"],
            update_data["grade_name"],
            update_data["origin_name"],
            update_data["origin_type_name"],
        ) = await get_or_create_all_other_tables(update_data_model, db)
        update_data = {
            k: v for k, v in update_data_model.model_dump().items() if v is not None
        }
        query = (
...
async def delete_crude_oil_imports(db: AsyncSession, filters: list) -> Optional[dict]:
    try:
        query = delete(CrudeOilImports).where(*filters).returning(CrudeOilImports)
        result = await db.execute(query)
...
async def add_to_db_before_commit(data, db):
    ...
    destination, destination_type, grade, origin, origin_type = (
        await get_or_create_all_other_tables(data, db)
    )
...
async def get_or_create_all_other_tables(data, db):
    ...
    if data.origin_name:
        origin = await get_or_create(db, Origin, name=data.origin_name)
    if data.origin_type_name:
        origin_type = await get_or_create(db, OriginType, name=data.origin_type_name)
    ...
    if data.grade_name:
        grade = await get_or_create(db, Grade, name=data.grade_name)
